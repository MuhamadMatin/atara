...
$sql = "";
if ($cari != "") {
    $sql = " SELECT `id`, `kd_kain`, `jenis_kain`,`client_nama`, `3_no_nota`, `3_date_entry`, `3_date_modified`, `3_date_transaction`, `3_date_return` FROM `stock` WHERE `3_date_transaction` IS NOT NULL AND `client_nama` like '%" . $cari . "%' ORDER BY `3_date_transaction` DESC;";
} else {
    $sql = "SELECT `id`, `kd_kain`, `jenis_kain`,`client_nama`, `3_no_nota`, `3_date_entry`, `3_date_modified`, `3_date_transaction`, `3_date_return` FROM `stock` WHERE `3_date_transaction` IS NOT NULL ORDER BY `3_date_transaction` DESC;";
}
...
if (mysqli_num_rows($hasil) == 0) {
    ...
    while ($row = mysqli_fetch_array($hasil)) {
        ...
        echo "> <i class='fas fa-undo fa-fw'></i></button>
        </td>";
        echo "</tr>";
    }
    echo "</table>";
